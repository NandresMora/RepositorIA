...
import { Layers, School, Briefcase, Store, Lightbulb, Trash2, Edit3, X as CloseIcon, Globe, Search, Plus } from 'lucide-react';
    ...
    <div className="flex flex-1 max-w-container-max mx-auto w-full bg-background min-h-[calc(100vh-64px)]">
      ...
      <aside className="hidden md:flex flex-col gap-6 p-8 w-64 bg-surface-container-low border-r border-outline-variant/30 shrink-0">
        ...
        <nav className="flex flex-col gap-6">
          <div className="flex flex-col gap-1">
            <p className="px-2 text-[9px] font-bold text-on-surface-variant uppercase tracking-widest mb-2 opacity-50">Dominios</p>
            <button 
            ...
            {(['Study', 'Work', 'Business'] as Pillar[]).map(pillar => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest font-geist">
                    {pillar === 'Study' ? 'Estudio' : pillar === 'Work' ? 'Trabajo' : 'Negocios'}
                  </span>
                </button>
              );
            })}
          ...
          <header className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-xs text-on-surface-variant font-medium opacity-60">
                Define los pilares estructurales del repositorio.
              </p>
            </div>
            ...
            >
                    ...
                    <div className="bg-surface-container-low border border-outline-variant/50 rounded-2xl p-8 shadow-2xl">
                      <div className="flex items-center justify-between mb-8">
                        <h2 className="text-xs font-bold font-geist text-on-surface uppercase tracking-[0.2em] flex items-center gap-2">
                          ...
                          {editingId ? 'Editar Unidad Estructural' : 'Inicializar Nueva Unidad'}
                        </h2>
                      ...
                      <form className="grid grid-cols-1 md:grid-cols-2 gap-8" onSubmit={handleSubmit}>
                        <div className="space-y-6">
                          <div>
                            <label className="text-[9px] font-bold text-on-surface-variant uppercase tracking-widest mb-2 block">Nombre de la Unidad</label>
                            <input 
                              value={form.name}
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="text-[9px] font-bold text-on-surface-variant uppercase tracking-widest mb-2 block">Referencia de Icono</label>
                              <select 
                                value={form.icon}
                              ...
                              >
                                <option value="Layers">Predeterminado</option>
                                <option value="psychology">Cerebro/IA</option>
                                <option value="settings">Sistema/Ajustes</option>
                                <option value="school">Académico</option>
                              </select>
                            </div>
                            <div>
                              <label className="text-[9px] font-bold text-on-surface-variant uppercase tracking-widest mb-2 block">Dominio</label>
                              <select 
                                value={form.pillar}
                        ...
                        <div className="space-y-6">
                          <div>
                            <label className="text-[9px] font-bold text-on-surface-variant uppercase tracking-widest mb-2 block">Propósito Técnico</label>
                            <textarea 
                              ...
                              placeholder="Breve alcance técnico..." 
                            />
                          </div>
                          <button className={`w-full py-4 rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] transition-all active:scale-[0.98] ${
                            ...
                            {editingId ? 'Confirmar Cambios' : 'Inicializar Unidad'}
                          </button>
                        </div>
                      </form>
                    </div>
              ...
              <div className="space-y-12">
                <div className="relative">
                  ...
                  <input 
                    type="text"
                    placeholder="Filtrar unidades técnicas..."
                      ...
                      return (
                        <motion.div key={pillarName} layout className="space-y-4">
                          <div className="flex items-center gap-3 px-2">
                            ...
                            <h3 className={`text-[10px] font-bold font-geist uppercase tracking-[0.3em] ${colorClass}`}>
                              {pillarName === 'Study' ? 'ESTUDIO' : pillarName === 'Work' ? 'TRABAJO' : 'NEGOCIOS'}
                            </h3>
                              ...
                              <div key={group.id} className="group flex items-center justify-between p-4 rounded-xl hover:bg-surface-container-low transition-all">
                                ...
                                <div className="flex items-center gap-6">
                                  <div className="text-right">
                                    ...
                                    <p className="text-[8px] font-bold text-on-surface-variant uppercase tracking-widest opacity-40">Activos</p>
                                  </div>
